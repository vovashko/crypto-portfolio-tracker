...
void printPortfolio(std::map<std::string, double> btcRate, std::ifstream &InFile, int containsHeader)
{
    ...
    while (getline(InFile, line))
    {
        ...
        if (getline(ss, date, '|') && getline(ss, valueStr))
        {
            trimString(date);
            trimString(valueStr);
            if (!verifyDate(date) || !verifyValue(valueStr))
                continue;
            double amount = std::stod(valueStr);
            if (btcRate.find(date) == btcRate.end())
            {
                std::string closestDate = findClosestLowerDate(btcRate, date);
                std::cout << date << "=> " << amount << " BTC = ($" << amount * btcRate[closestDate] << ")" << std::endl;
            }
            else
                std::cout << date << "=> " << amount << " BTC = ($" << amount * btcRate[date] << ")" << std::endl;
        }
        else
...
std::vector<std::string> loadCryptoPairs(const std::string &jsonFile)
{
    ...
    {
        cryptoPairs.push_back(pair.asString() + ".csv"); // Convert to CSV filename
    }
